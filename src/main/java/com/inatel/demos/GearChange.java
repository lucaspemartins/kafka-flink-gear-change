...
public class GearChange {
  ...
  public static void main(String[] args) throws Exception {
    ...
    stream  .flatMap(new TelemetryJsonParser())
            ...
            .flatMap(new GearChangeMapper())
            .map(new GearChangePrinter())
            .print();
    ...
    static class TelemetryJsonParser implements FlatMapFunction<ObjectNode, Tuple2<String, Integer>> {
      ...
      public void flatMap(ObjectNode jsonTelemetry, Collector<Tuple2<String, Integer>> out) throws Exception {
        ...
        float gearChange = jsonTelemetry.get("telemetry").get("Gear").floatValue();
        out.collect(new Tuple2<>(carNumber, Math.round(gearChange)));
      }
    }
    ...
    static class GearChangeReducer implements ReduceFunction<Tuple2<String, Integer>> {
      @Override
      public Tuple2<String, Integer> reduce(Tuple2<String, Integer> value1, Tuple2<String, Integer> value2) {
        return new Tuple2<>(value1.f0, Math.abs(value1.f1 - value2.f1));
      }
    }

    // FlatMap Function - Average
    // Calculates the average
    static class GearChangeMapper implements FlatMapFunction<Tuple2<String, Integer>, Tuple2<String, Integer>> {
      @Override
      public void flatMap(Tuple2<String, Integer> carInfo, Collector<Tuple2<String, Integer>> out) throws Exception {
        out.collect(  new Tuple2<>( carInfo.f0 , carInfo.f1 )  );
      }
    }

    // Map Function - Print average    
    static class GearChangePrinter implements MapFunction<Tuple2<String, Integer>, String> {
      @Override
      public String map(Tuple2<String, Integer> gearChangeEntry) throws Exception {
        return  String.format("Gear change for %s : %d ", gearChangeEntry.f0 , gearChangeEntry.f1 ) ;
      }
    }
